...
export function stripe(){return stripeInstance??=new Stripe(required('STRIPE_SECRET_KEY'))}
export function db(){return dbInstance??=new Pool({connectionString:required('DATABASE_URL'),max:10})}
